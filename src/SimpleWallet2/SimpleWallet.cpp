...
#include <SimpleWallet2/SimpleWallet.h>
#include <cstring>
    ...
    while (true) {
        ...
        if (address.length() != 95) {
            std::cout << WarningMsg("Address is wrong length!") << std::endl
                      << "It should be 95 characters long, but it is " << address.length() << " characters long!" << std::endl;
        } else if (address.substr(0, 1) != "6") {
            std::cout << WarningMsg("Invalid address! It should start with \"6\"!") << std::endl;
        } else if (!CryptoNote::parseAccountAddressString(prefix, publicKeys, address)) {
            std::cout << WarningMsg("Failed to parse B2B address! Ensure you have entered it correctly.") << std::endl;
        } else {
            break;
        }
    }
