  ...
  , logger(loggerGroup, "PaymentServiceJsonRpcServer")
{
  ...
  handlers.emplace("generatePaymentId", jsonHandler<GeneratePaymentId::Request, GeneratePaymentId::Response>(std::bind(&PaymentServiceJsonRpcServer::handleGeneratePaymentId, this, std::placeholders::_1, std::placeholders::_2)));
}
...
std::error_code PaymentServiceJsonRpcServer::handleGeneratePaymentId(const GeneratePaymentId::Request& request, GeneratePaymentId::Response& response) {
  return service.generatePaymentId(response.paymentId);
}
